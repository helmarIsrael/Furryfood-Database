...
class Ui_CU(object):
    ...
    def proceed(self):
        ...
        query2 = f"UPDATE `customer` SET `customer_ID` = '{code}', `customer_name` = '{name}', `contact_number` = '{number}', `birthday` = '{date}' WHERE `customer_ID` = '{code}'"
        mycursor.execute(query2)
        db.commit()

    def retranslateUi(self, Customer):
        _translate = QtCore.QCoreApplication.translate
        Customer.setWindowTitle(_translate("Customer", "Customer UPDATE"))

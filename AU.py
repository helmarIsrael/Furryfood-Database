...
class Ui_AU(object):
    ...
    def proceed(self):
        ...
        query2 = f"UPDATE `appointment` SET `appointment_ID` = '{code}', `appointment_date` = '{date}', `appointment_status` = '{status}', `customer_ID` = '{text}' WHERE `appointment_ID` = '{code}'"
        mycursor.execute(query2)
        db.commit()

    def retranslateUi(self, Appointment):
        _translate = QtCore.QCoreApplication.translate
        Appointment.setWindowTitle(_translate("Appointment", "Appointment UPDATE"))
